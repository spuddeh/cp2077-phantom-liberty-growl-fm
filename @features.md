# Features

## Implemented

- *Phantom Liberty* plays on Growl FM in full, streamed from the game's own audio. The mod ships no
  music.
- The credits render is used, trimmed of the silence at both ends, so the station opens on the song
  rather than on dead air and queues the next track on time.
- The station shows `Dawid Podsiadło, P.T. Adamczyk - Phantom Liberty`, in all nineteen languages.
- The track sits at Growl FM's own level and fades with distance like every other song, because the
  segment carries the station's broadcast sends, bus and dry Volume rather than a tuned trim.
- Coexists with Restore Nebula with no patch. The mod never starts the metadata load from
  `OnLoad`: it listens for the load, and takes a depot token only for a resource
  `AudioXLNative.IsResourceRequested` reports as already requested.
- AudioXL 0.4.3 or newer is a hard compile dependency.

## Planned

- At 347 s it is the longest track on the station by more than two minutes. Whether that reads as a
  centrepiece or as an outlier is a listening call, not a measurement.
- Russian and Ukrainian transliterate a Latin-script artist name in vanilla, and this ships the
  Latin string to every language. A native speaker could supply either.
